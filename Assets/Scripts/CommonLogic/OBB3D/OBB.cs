...
public class OBB : MonoBehaviour
{
    ...
    Vector3 P0 { get { return transform.localToWorldMatrix.MultiplyPoint3x4(new Vector3(-size.x * 0.5f, /*-size.y * 0.5f*/0, -size.y * 0.5f)); } }
    Vector3 P1 { get { return transform.localToWorldMatrix.MultiplyPoint3x4(new Vector3(size.x * 0.5f, /*-size.y * 0.5f*/0, -size.y * 0.5f)); } }
    ...
    Vector3 P4 { get { return transform.localToWorldMatrix.MultiplyPoint3x4(new Vector3(-size.x * 0.5f, /*-size.y * 0.5f*/0, size.y * 0.5f)); } }
    Vector3 P5 { get { return transform.localToWorldMatrix.MultiplyPoint3x4(new Vector3(size.x * 0.5f, /*-size.y * 0.5f*/0, size.y * 0.5f)); } }
    ...
    bool ProjectionIsNotIntersect(OBB x, OBB y, Vector3 axis)
    {
        ...
        var xMin = Mathf.Min(x_p0, Mathf.Min(x_p1, Mathf.Min(x_p4, x_p5)));
        var xMax = Mathf.Max(x_p0, Mathf.Max(x_p1, Mathf.Max(x_p4, x_p5)));
        var yMin = Mathf.Min(y_p0, Mathf.Min(y_p1, Mathf.Min(y_p4, y_p5)));
        var yMax = Mathf.Max(y_p0, Mathf.Max(y_p1, Mathf.Max(y_p4, y_p5)));

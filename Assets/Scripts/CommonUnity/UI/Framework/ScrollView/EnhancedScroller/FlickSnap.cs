...
namespace EnhancedScrollerDemos.FlickSnap
{
    ...
    public class FlickSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        public Action<int> OnJumpOver;

        public Action OnDragOver;

        public Action OnStartDrag;

        public Action OnDragIndexOverMaxSize;
        ...
        public int CurrentIdx => _currentIndex < MaxDataElements ? _currentIndex : MaxDataElements - 1;

        public void SimulationDrag(int jumpToIndex)
        {
            ...
            if (jumpToIndex != -1)
            {
                ...
                scroller.JumpToDataIndex(Mathf.Clamp(jumpToIndex, 0, MaxDataElements - 1), jumpComplete: () =>
                {
                    OnJumpOver?.Invoke(jumpToIndex);
        ...
        public void OnBeginDrag(PointerEventData data)
        {
            ...
            // Log.Info("cur index = {0}", _currentIndex);
        }
        ...
        public void OnEndDrag(PointerEventData data)
        {
            ...
            if (_isDragging)
            {
                ...
                else if(scroller.scrollDirection == EnhancedScroller.ScrollDirectionEnum.Vertical)
                {
                    if (delta.y < 0)
                    {
                        // if the change is less than zero, then we jump one element up (backward)
                ...
                if (jumpToIndex > MaxDataElements - 1)
                {
                    jumpToIndex = MaxDataElements - 1;
                    OnDragIndexOverMaxSize?.Invoke();
                }

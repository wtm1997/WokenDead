...
public class CollectUIData
{
    ...
    void TryGetAllElement(ref List<EElementType> typeList, Transform trans)
    {
        ...
        TryGetScrollViewComp(ref typeList, trans);
    ...
    void TryGetScrollViewComp(ref List<EElementType> typeList, Transform trans)
    {
        if (trans.TryGetComponent<EnhancedScroller>(out EnhancedScroller imageComp))
        {
            if (typeList != null)
            {
                if (!typeList.Contains(EElementType.ESCROLLVIEW))
                {
                    typeList.Add(EElementType.ESCROLLVIEW);
                }
            }
            else
            {
    ...
    string GetTransPath(Transform trans, ref bool isView, ref string viewName)
    {
        ...
        for (int i = reversePath.Count - 1; i >= 0; i--)
        {
            ...
            if (reversePath[i].Contains("v_"))
            {
                isView = true;
                res = "";
                viewName = reversePath[i].Split('_')[1];
                continue;
            }
